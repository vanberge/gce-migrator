...
SHOW_HELP() {
    ...
    echo "  Use format ./gce-migrate.sh <sourceproject> <destproject> <network> <migration-type>"
    echo "      <sourceproject>: The project where VM currently lives"
    echo "      <destproject>: The project where VM will reside after migration"
...
COUNT_ARGS() {
    if [ -z "$SOURCEPROJECT" ] || [ -z "$DESTPROJECT" ] || [ -z "$NETWORK" ] || [ -z "$METHOD" ]; then
        ERROR=1
...
CHECK_PROJECT() {
	gcloud projects list | grep -w $1 > /dev/null 2>&1
        if [ $? -ne 0 ]; then
            ERROR=1
...
CHECK_NETWORK() {
    gcloud compute networks list --project $DESTPROJECT | grep -w $NETWORK > /dev/null 2>&1
        if [ $? -ne 0 ]; then
            ERROR=1 
...
CHECK_METHOD() {
    case $METHOD in
        bulk)
            COMMAND() {
                gcloud compute instances list --project $SOURCEPROJECT | grep -w -v NAME | awk ' { print $1 } '
            }
        ;;
        list)
...
COUNT_ARGS
CHECK_PROJECT "$SOURCEPROJECT" 
CHECK_PROJECT "$DESTPROJECT" 
# CHECK_NETWORK  # Temporarily remove this
CHECK_METHOD
...
# Set Project IDs
SOURCEPROJECT_ID=$(gcloud projects list | grep -w $SOURCEPROJECT | awk ' { print $2 } ')
DESTPROJECT_ID=$(gcloud projects list | grep -w $DESTPROJECT | awk ' { print $2 } ')
...
if [ "$CURRENTPROJECT" != "$SOURCEPROJECT" ]; then
   gcloud config set project $SOURCEPROJECT
     if [ $? -ne 0 ]; then
       echo "Could not set project to $SOURCEPROJECT, exiting"
       ERROR_OUT
       exit
     fi
   else
      echo "Already in $SOURCEPROJECT, continuing!"
fi
...
DESTPROJECT_SVCACCT=$(gcloud iam service-accounts list --project $DESTPROJECT --filter="NAME=( 'Compute Engine default service account' )" | grep "Compute" | awk -F "  " '{ print $2 }') 
...
do
   ...
   gcloud beta compute instances create $VM \
    ...
    --private-network-ip=$IP --no-address
     if [ $? -ne 0 ]; then
        echo "ERROR: Could not create new instance of $VM in $DESTPROJECT"
        ERROR=1
